...
export default function Navbar() {
  ...
  return (
    <div className={`navbar fixed top-0 left-0 right-0 z-50 px-6 transition-all duration-300 ${
      scrolled ? "bg-choco/95 backdrop-blur-md shadow-lg shadow-black/40" : "bg-transparent"
    }`}>
      ...
      <div className="navbar-end hidden lg:flex gap-1">
        {links.map((l) => (
          <button key={l.href} onClick={() => handleNav(l.href)}
            className={`btn btn-xs rounded-full border-none font-medium transition-all duration-300 ${
              active === l.href
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 right-0 bg-choco/98 backdrop-blur-md flex flex-col items-center gap-2 py-4 shadow-xl border-t border-catawba/30">
          {links.map((l) => (
            <button key={l.href} onClick={() => handleNav(l.href)}
              className={`btn btn-sm w-48 rounded-full border-none font-medium transition-all duration-300 ${
                active === l.href
                  ? "bg-catawba text-pearl"
